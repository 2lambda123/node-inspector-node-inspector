...
var fs = require('fs'), path = require('path'),
    inherits = require('util').inherits, extend = require('util')._extend,
    EventEmitter = require('events').EventEmitter, async = require('async'),
...
extend(PageAgent.prototype, {
  enable : function(params, done) { done(); },

  canEmulate : function(params, done) { done(null, {result : false}); },

  canScreencast : function(params, done) { done(null, {result : false}); },

  setTouchEmulationEnabled : function(params, done) { done(); },
  ...
  getResourceTree : function(params, done) {
    ...
    if (this._debuggerClient.isReady) {
      ...
      this._debuggerClient.once('connect',
  ...
  _getResourceTreeForAppScript : function(target, startDirectory, mainAppScript,
                                          done) {
    async.waterfall(
        [
          this._scriptStorage.findAllApplicationScripts.bind(
              this._scriptStorage, startDirectory, mainAppScript),
          this._createResourceTreeResponse.bind(this, target, mainAppScript)
        ],
        done);
  },

  _createResourceTreeResponse : function(target, mainAppScript, scriptFiles,
                                         done) {
  ...
  getResourceContent : function(params, done) {
    var scriptName = convert.inspectorUrlToV8Name(
        params.url,
    ...
    if (scriptName === '') {
      ...
      return process.nextTick(
          this._convertScriptSourceToGetResourceResponse.bind(this, content,
                                                              done));
    }
  ...
  _convertScriptSourceToGetResourceResponse : function(
      source, done) { return done(null, {content : source}); },

  reload : function(params, done) {
    // This is called when user press Cmd+R (F5?), do we want to perform an
    // action on this?
    done();
  }
});
